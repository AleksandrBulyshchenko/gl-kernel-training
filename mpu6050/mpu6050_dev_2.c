...
struct mpu6050_dev_context {
	...
	struct work_struct read_struct;
...
static void mpu6050_read_func(struct work_struct *ws)
{
...
void IRQ_TimerHandler(unsigned long data)
{
	...
	schedule_work(&(context->read_struct));
	(context->read_timer).expires = get_jiffies_64() + period_ms/(1000/HZ);
	add_timer(&(context->read_timer));
	i++;
}
...
static int mpu6050_dev_init(void) {
	...
	INIT_WORK(&(context->read_struct), mpu6050_read_func);
	init_timer(&(context->read_timer));
	(context->read_timer).expires = get_jiffies_64() + period_ms/(1000/HZ);
	(context->read_timer).data = 0;
	(context->read_timer).function = IRQ_TimerHandler;
	add_timer(&(context->read_timer));
err:
	return ret;
}
